...
def RansacSolveP3P(points2D, points3D, cameraMatrix, distCoeffs, n_iter=10, thres=1):
    ...
    print(points2D[not_select])
    print(abs(coordinates_2D[:,:2] - points2D[not_select]).sum(axis=1))
    print((abs(coordinates_2D[:,:2] - points2D[not_select]).sum(axis=1)<thres).sum())
    #print(selected_T)
    #print(selected_Quat)